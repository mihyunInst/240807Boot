...
Ajax(Asynchronous JavaScript and XML)를 사용할 때 Promise는 비동기 작업의 결과를 처리하는 방법 중 하나입니다. 
쉽게말해서 비동기 작업이 맞이할 미래의 완료 또는 실패와 그 결과 값을 나타냄 
-> 어떤 미래가 올지는 모르지만 결과를 보내주겠단 약속
Promise는 비동기 작업이 완료되었을 때 실행할 콜백 함수를 지정하고, 해당 작업의 성공 또는 실패 여부를 처리할 수 있도록 합니다.

Promise는 주로 Ajax 요청과 같은 비동기 작업을 처리하는 데 사용됩니다.
예를 들어, 웹 페이지에서 사용자의 입력에 따라 서버와 통신하여 데이터를 가져오는 경우가 있습니다. 
이때, 서버로부터 응답이 도착하기까지 시간이 걸리므로 비동기적으로 처리해야 합니다.

Promise를 사용하면 비동기 작업의 성공 또는 실패에 대한 처리를 쉽게 할 수 있습니다. 
Promise 객체는 세 가지 상태를 가질 수 있습니다:

Pending(대기 중): 비동기 작업이 완료되지 않은 상태입니다.(이행하지도, 거부하지도 않은 초기 상태)
...
function getTotalCount(){ // 함수 정의
  ...
  .then( response => {
    ...
    //console.log(response.text()); // 찍어보면 Promise의 pending상태로 나옴
